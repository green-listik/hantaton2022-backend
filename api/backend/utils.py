...
async def get_dots(db: AsyncSession, event_id: int) -> Dots | None:
    ...
    for operation in event.operations:
        planned_depth = float(operation.parameters.get('planned_depth'))
        actual_depth = float(operation.parameters.get('actual_depth'))
        planned_day = float(operation.parameters.get('planned_time')) / 24 + planned_day
        actual_day = float(operation.parameters.get('actual_time')) / 24 + actual_day
